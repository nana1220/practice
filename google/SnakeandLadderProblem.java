...
For example consider the board shown on right side (taken from here), the minimum number of dice throws required to reach
cell 30 from cell 1 is 3. Following are steps.
...
to next six vertices if next 6 vertices do not have a snake or ladder. If any of the next six vertices has a snake or
ladder, then the edge from current vertex goes to the top of the ladder or tail of the snake. Since all edges are of equal
weight, we can efficiently find shortest path using Breadth First Search of the graph.
 */
...
class Solu{
  ...
  struct queueEntry
  {
    int v;     // Vertex number, value of vertex
    int dist;  // Distance of this vertex from source, store distance(number of throws) in the vertex
  };
...
// takes to.
  int getMinDiceThrows(int move[], int N)
  {
    ...
    queueEntry qe;  // A queue entry (qe): result: defined outside the loop
    while (!q.empty())
    {
      qe = q.front();
      ...
      // we are done
      if (v == N-1) // reach the destination
        break;
      ...
      q.pop();
      for (int j=v+1; j<=(v+6) && j<N; ++j) // 6 adjacents
      {
        ...
        if (!visited[j])
        {
          ...
          // as visited
          queueEntry a;
          a.dist = (qe.dist + 1); // update distance
          visited[j] = true;
          ...
          if (move[j] != -1)
            a.v = move[j]; // update value of the vertex
          else
            a.v = j; // if not snake or ladder
          q.push(a);
        }
      }
    }
