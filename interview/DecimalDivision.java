public class DecimalDivision {
  ...
  public String divide(int a, int b) {
    ...
    while (remainder != 0 && remainders.size() <= PRECISE) {
      ...
      res = a / b;
      sb.append(res);
      int len = getLengthOfRecurringPart(remainders, a % b); // can use a hashmap to store remainder and its index (res.length())
      if (len > 0) {
        sb.insert(sb.length() - len + 1, "("); // StringBuilder can insert
        sb.append(")");
        return sb.toString();
      }
      remainder = a % b;
