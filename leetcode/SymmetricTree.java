...
public class Solution {
  public boolean isSymmetric(TreeNode root) {
    ...
    while (!qu.isEmpty()){      // compare one pair each time
      ...
      if (t1==null || t2==null || t1.val != t2.val)   return false; // NOTE: check if only one of them is null
      qu.add(t1.left);
      qu.add(t2.right);
      qu.add(t1.right);
      qu.add(t2.left);
...
public class Solution {
  ...
  public void inorderTraversal(TreeNode n, ArrayList<String> nodes){
    if (n == null){
      nodes.add("#"); //NOTE!!!: this is to fill the leaf with "#" if its parent only has either left child or right child
      return;
    }
    if (n.left==null && n.right==null)  nodes.add(""+n.val); //NOTE: for node that has neither left nor right, fill this leaf with value
    else{
