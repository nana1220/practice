...
public class StringToInteger {
  ...
  public int atoi(String str) {
    ...
    int res = 0;
    int flow = sign == -1 ? Integer.MIN_VALUE : Integer.MAX_VALUE;
    for (int i = 0; i < str.length(); i++) {
      ...
      int digit = ch - '0';
      // check overflow
      // NOTE: if check with res * 10 + digit > overflow,
      // in the case of overflow res * 10 + digit has been overflow before check >
      if (sign == 1 && (flow - digit) / 10 < res) {
        return flow;
        // underflow
        // res * 10 - digit < overflow
      } else if (sign == -1 && (flow + digit) / 10 > res) {
        return flow;
      }
